...
namespace Mapbox.Unity.MeshGeneration.Data
{
    ...
    public class UnityTile : MonoBehaviour, IEquatable<UnityTile>
	{
		...
		internal void Initialize(IMapReadable map, UnwrappedTileId tileId, float scale, int zoom, Texture2D loadingTexture = null)
		{
			...
			//use a class outside of this namespace 
		...
		private CanonicalTileId _canonicalTileId;		
		public void SetRasterCacheData(CanonicalTileId key, byte[] data, bool useMipMap = true, bool useCompression = false)
		{
			_canonicalTileId = key;
			if (RasterDataState != TilePropertyState.Unregistered)
			{
				...
				if(TileManager.IsTileCached(key))
				{
					// _rasterData = TileManager.GetTileTexture(key);
					// MeshRenderer.sharedMaterial.mainTexture = _rasterData;
					// RasterDataState = TilePropertyState.Loaded;
					//tile is already cached, don't do anything
					var texture = TileManager.GetTileTexture(key);
					_rasterData = texture;
		...
		public bool Equals(UnityTile other)
		{
			if (other == null)
			{
				return false;
